 ...
 * When run in reverse, these motors power the robot's climber through a 1 way bearing.
 * The shooter subsystem goes through 3 stages when shooting.
 * 1. Spin Up
 *  Use a PIDF controller to spin up to the desired RPM.  We acquire this desired RPM
 *  by converting the camera's range value into an RPM value using the range map in the
 *  {@link Constants} class.
 * 2. Hold When Ready
 *  Once the flywheel's RPM stabilizes (remains within a certain bandwidth for certain
 *  amount of time), the shooter switches to the hold when ready stage.  In this stage,
 *  we collect kF samples.  The idea is that we want to run the shooter in open loop 
 *  when we start firing, so in this stage we calculate the voltage we need to supply 
 *  to spin the flywheel at the desired RPM.
 * 3. Hold
 *  Once we collect enough kF samples, the shooter switches to the hold stage.  This is
 *  the stage that we begin firing balls.  We set kP, kI, and kD all to 0 and use the kF
 *  value we calculated in the previous stage for essentially open loop control.  
 *  The reason we fire in open loop is that we found it creates a much narrower stream 
 *  and leads to smaller RPM drops when fuel is fired.
 *  
 * @see Subsystem.java
 */
