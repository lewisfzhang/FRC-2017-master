...
public class TrajectoryFollower {
    ...
    /*
    public MotionState updateSetpoint(double max_vel, double max_acc, final MotionState prev_state, double current_time) {
        max_vel = Math.abs(max_vel);
        max_acc = Math.abs(max_acc);
        setpoint_ = new MotionState();
        setpoint_.t = current_time;
        final double dt = Math.max(0.0, current_time - prev_state.t);
        if (isFinishedTrajectory()) {
            setpoint_.pos = goal_state_.pos;
            setpoint_.vel = goal_state_.vel;
            setpoint_.acc = 0;
            return setpoint_;
        }
        // Compute the new commanded position, velocity, and acceleration.
        double distance_to_go = goal_state_.pos - prev_state.pos;
        double cur_vel = prev_state.vel;
        double cur_vel2 = cur_vel * cur_vel;
        double goal_vel = goal_state_.vel;
        // In cases where the goal is opposite the current direction of travel, it's easiest just to invert everything and solve the positive distance case.
        ...
        double max_reachable_velocity_disc = cur_vel2 / 2.0 + max_acc * distance_to_go;
        double min_reachable_velocity_disc = cur_vel2 / 2.0 - max_acc * distance_to_go;
        double cruise_vel = cur_vel;
        if (min_reachable_velocity_disc < 0 || cruise_vel < 0) {
            cruise_vel = Math.min(max_vel, Math.sqrt(max_reachable_velocity_disc));
        }
        double t_start = (cruise_vel - cur_vel) / max_acc; // Accelerate
        // to
        // cruise_vel
    ...
    */
    ...
    public boolean isFinishedTrajectory() {
        return Math.abs(setpoint_.pos - goal_state_.pos) < goal_state_.pos_tolerance;// && (Math.abs(setpoint_.vel) < goal_state_.vel_tolerance  ;
    }
}
